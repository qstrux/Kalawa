...
(function($){'use strict';$.fn.xpanda=function(settings){var self=this,defaults=$.extend(true,{breakpoints:{xs:{minWidth:0,itemsPerRow:2},sm:{minWidth:576,itemsPerRow:4},md:{minWidth:768,itemsPerRow:6},lg:{minWidth:992,itemsPerRow:8},xl:{minWidth:1200,itemsPerRow:10},xx:{minWidth:1400,itemsPerRow:12}},element:{selector:'.x-item',equalThumbSizes:true,wrapperClass:'x-item-wrap',toClone:'.x-content',spaceBetweenItems:10,spaceBetweenRows:10},placeholder:{class:'x-placeholder',marginSides:10,desktopScrollTo:true,scrollSpeed:400,scrollTopOffset:0,slideAnimation:true,slideSpeed:400,showControls:true},spacer:{class:'x-spacer-inside'},independent:true,history:false},settings);return self.each(function(){var APP={};APP.GO={globalVar:function(){APP.GO.win=$(window);APP.GO.doc=$(document);APP.GO.html=$('html');APP.GO.userAgent=navigator.userAgent;APP.GO.container=self;APP.GO.spacer=$('.x-spacer-outside');APP.GO.item=APP.GO.container.find(String(defaults.element.selector));APP.GO.itemLength=APP.GO.item.length;APP.GO.winWidth=APP.GO.win.width();APP.GO.winHeight=APP.GO.win.height();APP.GO.winTop=APP.GO.win.scrollTop();APP.GO.containerWidth=APP.GO.container.width();APP.GO.containerWidthCached=APP.GO.container.width();APP.GO.columnCount=0;APP.GO.updatedColumnCount=0;APP.GO.placeholderCount=1;APP.GO.throttleResizeFunc=_.throttle(APP.GO.updateResizeThrottle,1);APP.GO.throttleScrollFunc=_.throttle(APP.GO.updateScrollThrottle,10);},initialLoad:function(){var uaString=APP.GO.userAgent.toLowerCase();APP.GO.isMobileDevice=(uaString.match(/ipad|iphone|android|iemobile/g))?true:false;$('.xpanda[data-instance="1"]').addClass('x-keyboard-active');$('.xpanda').on('click',function(){$(this).addClass('x-keyboard-active').siblings('.xpanda').removeClass('x-keyboard-active');});APP.GO.fixWidth=(APP.GO.container.hasClass('x-fix-width'))?true:false;APP.GO.container.addClass('x-initiated');APP.GO.spacer.addClass('x-initiated');APP.GO.jqueryVersion=$.fn.jquery.split('.')[0];},destroyPreloader:function(){APP.GO.container.each(function(){var $self=$(this);if($self.hasClass('x-preloader')){$self.removeClass('x-preloader').promise().then(function(){setTimeout(function(){APP.GO.lazyLoadThumb();},10);});}});},getColumns:function(){if(APP.GO.containerWidth>defaults.breakpoints.xx.minWidth){APP.GO.columnCount=defaults.breakpoints.xx.itemsPerRow;}else if(APP.GO.containerWidth<=defaults.breakpoints.xx.minWidth&&APP.GO.containerWidth>defaults.breakpoints.xl.minWidth){APP.GO.columnCount=defaults.breakpoints.xl.itemsPerRow;}else if(APP.GO.containerWidth<=defaults.breakpoints.xl.minWidth&&APP.GO.containerWidth>defaults.breakpoints.lg.minWidth){APP.GO.columnCount=defaults.breakpoints.lg.itemsPerRow;}else if(APP.GO.containerWidth<=defaults.breakpoints.lg.minWidth&&APP.GO.containerWidth>defaults.breakpoints.md.minWidth){APP.GO.columnCount=defaults.breakpoints.md.itemsPerRow;}else if(APP.GO.containerWidth<=defaults.breakpoints.md.minWidth&&APP.GO.containerWidth>defaults.breakpoints.sm.minWidth){APP.GO.columnCount=defaults.breakpoints.sm.itemsPerRow;}else if(APP.GO.containerWidth<=defaults.breakpoints.sm.minWidth&&APP.GO.containerWidth>defaults.breakpoints.xs.minWidth){APP.GO.columnCount=defaults.breakpoints.xs.itemsPerRow;}else{console.log('error with breakpoints');}},createPlaceholders:function(){APP.GO.container.each(function(){var thisInstance=$(this),thisInstanceItem=thisInstance.find(defaults.element.selector),thisInstanceItemLength=thisInstanceItem.length;APP.GO.updatedColumnCount=APP.GO.columnCount;APP.GO.placeholderCount=1;thisInstanceItem.each(function(){var $self=$(this),itemIndex=thisInstanceItem.index($self),remainder=itemIndex%APP.GO.updatedColumnCount,dataUntil=APP.GO.updatedColumnCount*APP.GO.placeholderCount,indicator='<div class="x-indicator"></div>',placeholderMarkup=$('<div class="'+defaults.placeholder.class+'" data-until="'+dataUntil+'">').prepend(indicator).append('<div class="'+defaults.placeholder.class+'-inner" style="top: '+defaults.element.spaceBetweenRows+'px;"></div>'),horiMargins=(APP.GO.fixWidth)?0:(defaults.element.spaceBetweenItems/2);$self.attr('data-index',itemIndex+1).attr('data-group',dataUntil);if(remainder===(APP.GO.updatedColumnCount-1)||itemIndex===(thisInstanceItemLength-1)){$self.after(placeholderMarkup);thisInstance.find(defaults.element.selector+'[data-group="'+dataUntil+'"]').wrapAll('<div data-columns="'+APP.GO.updatedColumnCount+'" class="'+defaults.element.wrapperClass+'" style="margin: 0 '+horiMargins+'px"></div>');APP.GO.placeholderCount=APP.GO.placeholderCount+1;}});APP.GO.createButtons(false);APP.GO.onArrowClick();APP.GO.onCloseClick();});},destroyPlaceholders:function(){APP.GO.container.each(function(){var thisInstance=$(this),thisInstanceItem=thisInstance.find(defaults.element.selector);thisInstanceItem.unwrap();thisInstance.find('.'+defaults.placeholder.class).remove().removeAttr('style');});},styleItems:function(){APP.GO.item.each(function(){var $self=$(this),thisIndex=$self.attr('data-index'),img=$self.find('img'),iWidth=img.width(),iHeight=img.height(),flexValue=iWidth/iHeight,hori=(defaults.element.spaceBetweenItems/2)+'px',firstAndLastHori=(APP.GO.fixWidth)?0:hori,vert=defaults.element.spaceBetweenRows+'px';if(APP.GO.updatedColumnCount===1||defaults.element.equalThumbSizes===true){$self.css({'flex-grow':1});}else{$self.css({'flex-grow':flexValue});}
$self.css({'flex-shrink':1,'flex-basis':'0%','margin-top':vert,'margin-bottom':0});if(thisIndex%APP.GO.updatedColumnCount===0){$self.css({'margin-right':firstAndLastHori,'margin-left':hori});}else if(thisIndex%APP.GO.updatedColumnCount-1===0){$self.css({'margin-right':hori,'margin-left':firstAndLastHori});}else{$self.css({'margin-right':hori,'margin-left':hori});}}).promise().then(function(){APP.GO.createSpacer();});APP.GO.container.css('margin-bottom',defaults.element.spaceBetweenRows);},createSpacer:function(){APP.GO.container.each(function(){var thisInstance=$(this),thisDataInstance=thisInstance.attr('data-instance'),originalSpacer=$('.x-spacer-outside[data-instance="'+thisDataInstance+'"]'),createdSpacer=$('.'+defaults.spacer.class+'[data-instance="'+thisDataInstance+'"]'),thisInstanceItem=thisInstance.find(defaults.element.selector),thisInstanceItemLength=thisInstanceItem.length;originalSpacer.hide();createdSpacer.remove();thisInstanceItem.each(function(){var itemIndex=thisInstanceItem.index(this),lastPlaceholderUntil=thisInstance.find('.'+defaults.placeholder.class).last().attr('data-until'),lastRowItems=thisInstance.find(defaults.element.selector+'[data-group="'+lastPlaceholderUntil+'"]'),lastRowItemsLength=lastRowItems.length,spacesToFill=APP.GO.columnCount-lastRowItemsLength,paddingFix=((spacesToFill-1)*defaults.element.spaceBetweenItems)/2,lastRow=thisInstance.find('.'+defaults.element.wrapperClass).last(),appendedMarginRight=(APP.GO.fixWidth)?0:(defaults.element.spaceBetweenItems/2),originalWidth=(APP.GO.fixWidth)?APP.GO.containerWidth:APP.GO.containerWidth-(defaults.element.spaceBetweenItems*2),originalMargins=(APP.GO.fixWidth)?0:defaults.element.spaceBetweenItems,spacerMarkup=$('<div/>',{'class':defaults.spacer.class}).css({'flex-grow':spacesToFill,'flex-shrink':1,'flex-basis':'0%','margin-top':defaults.element.spaceBetweenRows+'px','margin-left':(defaults.element.spaceBetweenItems/2),'margin-right':appendedMarginRight,'padding-left':paddingFix,'padding-right':paddingFix}).attr('data-instance',thisDataInstance);if(itemIndex===(thisInstanceItemLength-1)){if(lastRowItemsLength<APP.GO.columnCount){spacerMarkup.appendTo(lastRow);}else{originalSpacer.show().css({'width':originalWidth+'px','margin-left':originalMargins+'px','margin-right':originalMargins+'px','margin-bottom':defaults.element.spaceBetweenRows+'px'});}}});}).promise().then(function(){APP.GO.addSpacerContent();});},addSpacerContent:function(){APP.GO.container.each(function(){var thisInstance=$(this),thisDataInstance=thisInstance.attr('data-instance'),spacerWrap=$('.x-spacer-outside[data-instance="'+thisDataInstance+'"]'),spacerContent=spacerWrap.html();function isEmpty(el){return!$.trim(el.html());}
if(isEmpty(spacerWrap)){thisInstance.find('.'+defaults.spacer.class).addClass('x-spacer-empty');}else{thisInstance.find('.'+defaults.spacer.class).html(spacerContent);}}).promise().then(function(){APP.GO.destroyPreloader();APP.GO.lazyLoadThumb();});},updateResizeThrottle:function(){APP.GO.containerWidth=APP.GO.container.width();APP.GO.container.each(function(){var thisInstance=$(this),currentActive=thisInstance.find('.x-is-active'),dataIndex=currentActive.attr('data-index'),thisInstanceInner=thisInstance.find('.'+defaults.placeholder.class+'-inner'),placeholder=thisInstance.find('.'+defaults.placeholder.class);thisInstanceInner.css({'display':''});if(APP.GO.updatedColumnCount!==APP.GO.columnCount){APP.GO.destroyPlaceholders();APP.GO.createPlaceholders();APP.GO.styleItems();}else{if(placeholder.hasClass('x-is-expanded')){thisInstance.find('.x-is-expanded > .x-placeholder-inner').css('height','auto');}
APP.GO.createSpacer();}
if(currentActive.length>0){if(APP.GO.containerWidth!==APP.GO.containerWidthCached){APP.GO.containerWidthCached=APP.GO.container.width();thisInstance.find('.x-is-expanded > .x-placeholder-inner').find('.x-asset, .x-info, .x-updated-content').remove();APP.GO.openXpanda(currentActive,dataIndex,false,false,false);}
APP.GO.styleActivePlaceholder();}
APP.GO.getColumns();});},updateScrollThrottle:function(){APP.GO.winHeight=APP.GO.win.height();APP.GO.winTop=APP.GO.win.scrollTop();APP.GO.lazyLoadThumb();},onItemClick:function(){APP.GO.item.on('click',function(e){e.preventDefault();var slide=defaults.placeholder.slideAnimation,scroll=defaults.placeholder.desktopScrollTo,indi=defaults.independent,$self=$(this),historyPath=$self.attr('data-path'),instanceSiblings=$self.closest('.xpanda').find(defaults.element.selector),allSiblings=$('.xpanda').find(defaults.element.selector),instanceActiveSibling=$self.closest('.xpanda').find('.x-is-active'),allActiveSibling=$('.xpanda').find('.x-is-active'),selfIndex=$self.attr('data-index'),instanceSiblingIndex=instanceActiveSibling.attr('data-index'),allSiblingIndex=allActiveSibling.attr('data-index'),selfGroup=$self.attr('data-group'),instanceSiblingGroup=instanceActiveSibling.attr('data-group');function clearHistory(){var url=window.location.href.toString(),query=window.location.search.toString();if(query.includes('?xpnd=')){var originalUrl=url.split('?xpnd=')[0];}else{var originalUrl=url.split('&xpnd=')[0];}
history.replaceState(null,null,originalUrl);}
if($self.hasClass('x-is-active')){if(indi===false){allSiblings.removeClass('x-is-not-active');}else{instanceSiblings.removeClass('x-is-not-active');}
$self.removeClass('x-is-active');if(slide===true){APP.GO.closeXpanda($self,selfIndex,true);}else{APP.GO.closeXpanda($self,selfIndex,false);}
if(defaults.history){clearHistory();}}else{if(indi===false){allSiblings.removeClass('x-is-active').addClass('x-is-not-active');}else{instanceSiblings.removeClass('x-is-active').addClass('x-is-not-active');}
$self.removeClass('x-is-not-active').addClass('x-is-active');if(instanceActiveSibling.length>0||allActiveSibling.length>0){if(selfGroup===instanceSiblingGroup){if(slide===true){APP.GO.adjustXpanda($self,instanceActiveSibling,selfIndex,true);}else{APP.GO.adjustXpanda($self,instanceActiveSibling,selfIndex,false);}}else{if(slide===true){if(scroll===true&&APP.GO.isMobileDevice===false){if(indi===false){APP.GO.closeXpanda(allActiveSibling,allSiblingIndex,true);}else{APP.GO.closeXpanda(instanceActiveSibling,instanceSiblingIndex,true);}
setTimeout(function(){APP.GO.openXpanda($self,selfIndex,true,true);},defaults.placeholder.slideSpeed+50);}else{if(indi===false){APP.GO.closeXpanda(allActiveSibling,allSiblingIndex,true);}else{APP.GO.closeXpanda(instanceActiveSibling,instanceSiblingIndex,true);}
setTimeout(function(){APP.GO.openXpanda($self,selfIndex,true,false,true);},defaults.placeholder.slideSpeed+50);}}else{if(indi===false){APP.GO.closeXpanda(allActiveSibling,allSiblingIndex,false);}else{APP.GO.closeXpanda(instanceActiveSibling,instanceSiblingIndex,false);}
if(scroll===true&&APP.GO.isMobileDevice===false){APP.GO.openXpanda($self,selfIndex,false,true,true);}else{APP.GO.openXpanda($self,selfIndex,false,false,true);}}}}else{if(slide===true){if(scroll===true&&APP.GO.isMobileDevice===false){APP.GO.openXpanda($self,selfIndex,true,true,false);}else{APP.GO.openXpanda($self,selfIndex,true,false,false);}}else{if(scroll===true&&APP.GO.isMobileDevice===false){APP.GO.openXpanda($self,selfIndex,false,true,false);}else{APP.GO.openXpanda($self,selfIndex,false,false,false);}}}
if(defaults.history){clearHistory();var query=window.location.href.indexOf('?')===-1?'?xpnd=':'&xpnd=';history.pushState(null,null,window.location.href+query+historyPath);}}});},openXpanda:function(target,index,slide,scroll,sibling){var content=target.find(defaults.element.toClone).html(),closestInstance=target.closest('.xpanda'),ceiling=Math.ceil(index/APP.GO.updatedColumnCount)*APP.GO.updatedColumnCount,cloneDestination=closestInstance.find('.'+defaults.placeholder.class+'[data-until="'+ceiling+'"]'),cloneDestinationInner=cloneDestination.find('.'+defaults.placeholder.class+'-inner'),placeholderTop=defaults.element.spaceBetweenRows,scrollTopOffset=defaults.placeholder.scrollTopOffset;cloneDestination.addClass('x-is-expanded');cloneDestinationInner.prepend(content).promise().then(function(){if(APP.GO.jqueryVersion<3){APP.GO.styleActivePlaceholder();}
cloneDestinationInner.css({height:'auto',top:placeholderTop}).promise().then(function(){var adjustedHeight=cloneDestinationInner.outerHeight();if(sibling===true){if(slide===true){cloneDestinationInner.animate({height:adjustedHeight});cloneDestination.animate({height:adjustedHeight+defaults.element.spaceBetweenRows},defaults.placeholder.slideSpeed,'linear').promise().then(function(){cloneDestinationInner.animate({opacity:1}).promise().then(function(){if(scroll===true){$('html, body').animate({scrollTop:target.parent('.x-item-wrap').offset().top-scrollTopOffset},defaults.placeholder.scrollSpeed,'linear');APP.GO.showIndicator(target,100);}else{APP.GO.showIndicator(target,0);}});});}else{cloneDestination.css({height:adjustedHeight+defaults.element.spaceBetweenRows});cloneDestinationInner.css({height:adjustedHeight,opacity:1}).promise().then(function(){if(scroll===true){$('html, body').animate({scrollTop:target.parent('.x-item-wrap').offset().top-scrollTopOffset},defaults.placeholder.scrollSpeed,'linear');APP.GO.showIndicator(target,400);}else{APP.GO.showIndicator(target,200);}});}}else{if(slide===true){cloneDestinationInner.animate({height:adjustedHeight});cloneDestination.animate({height:adjustedHeight+defaults.element.spaceBetweenRows},defaults.placeholder.slideSpeed,'linear').promise().then(function(){cloneDestinationInner.animate({opacity:1}).promise().then(function(){if(scroll===true){$('html, body').animate({scrollTop:target.parent('.x-item-wrap').offset().top-scrollTopOffset},defaults.placeholder.scrollSpeed,'linear');APP.GO.showIndicator(target,0);}else{APP.GO.showIndicator(target,0);}});});}else{cloneDestination.css({height:adjustedHeight+defaults.element.spaceBetweenRows});cloneDestinationInner.css({height:adjustedHeight,opacity:1}).promise().then(function(){if(scroll===true){$('html, body').animate({scrollTop:target.parent('.x-item-wrap').offset().top-scrollTopOffset},defaults.placeholder.scrollSpeed,'linear');APP.GO.showIndicator(target,400);}else{APP.GO.showIndicator(target,200);}});}}});if(APP.GO.jqueryVersion>=3){APP.GO.styleActivePlaceholder();}
APP.GO.lazyLoadAsset(index);APP.GO.updateButtons(index,ceiling);});},closeXpanda:function(target,index,slide){var closestInstance=target.closest('.xpanda'),ceiling=Math.ceil(index/APP.GO.updatedColumnCount)*APP.GO.updatedColumnCount,cloneDestination=closestInstance.find('.'+defaults.placeholder.class+'.x-is-expanded'),cloneDestinationInner=cloneDestination.find('.'+defaults.placeholder.class+'-inner');APP.GO.hideIndicator();if(slide===true){cloneDestination.animate({height:0},defaults.placeholder.slideSpeed,'linear').promise().then(function(){cloneDestination.removeClass('x-is-expanded').removeAttr('style');});cloneDestinationInner.animate({height:0,opacity:0},defaults.placeholder.slideSpeed,'linear').promise().then(function(){cloneDestinationInner.removeAttr('style').find('.x-asset, .x-info, .x-updated-content').remove();});}else{cloneDestination.removeClass('x-is-expanded').removeAttr('style');cloneDestinationInner.removeAttr('style').css({opacity:0}).find('.x-asset, .x-info, .x-updated-content').remove();}
APP.GO.lazyLoadThumb();},adjustXpanda:function(target,targetSibling,targetIndex,slide){var content=target.find(defaults.element.toClone).html(),closestInstance=targetSibling.closest('.xpanda'),ceiling=Math.ceil(targetIndex/APP.GO.updatedColumnCount)*APP.GO.updatedColumnCount,cloneDestination=closestInstance.find('.'+defaults.placeholder.class+'[data-until="'+ceiling+'"]'),cloneDestinationInner=cloneDestination.find('.'+defaults.placeholder.class+'-inner'),slideSpeed,indicatorDelay;APP.GO.hideIndicator();if(slide===true){slideSpeed=defaults.placeholder.slideSpeed;indicatorDelay=0;}else{slideSpeed=0;indicatorDelay=200;}
cloneDestinationInner.find('.x-info, .x-asset, .x-updated-content').animate({opacity:0},slideSpeed,'linear').promise().then(function(){APP.GO.updateButtons(targetIndex,ceiling);cloneDestinationInner.find('.x-asset, .x-info, .x-updated-content').remove();cloneDestinationInner.prepend('<div class="x-updated-content" style="opacity: 0;">'+content+'</div>').promise().then(function(){cloneDestinationInner.css({'display':'block'});var infoHeight=cloneDestinationInner.find('.x-updated-content').outerHeight();cloneDestination.animate({height:infoHeight+defaults.element.spaceBetweenRows},slideSpeed,'linear');cloneDestinationInner.animate({height:infoHeight},slideSpeed,'linear').promise().then(function(){cloneDestinationInner.find('.x-info, .x-asset, .x-updated-content').animate({opacity:1},slideSpeed,'linear').promise().then(function(){APP.GO.showIndicator(target,indicatorDelay);APP.GO.styleActivePlaceholder();APP.GO.lazyLoadAsset(targetIndex);});});});});APP.GO.lazyLoadThumb();},showIndicator:function(target,delay){var tPosition=target.position(),tWidth=target.width(),tLeft=tPosition.left+(tWidth/2),indicator=target.parent('.x-item-wrap').next('.'+defaults.placeholder.class).find('.x-indicator'),iWidth=indicator.width(),iHeight=indicator.height();indicator.css({'left':tLeft-(iWidth/2),'top':defaults.element.spaceBetweenRows}).promise().then(function(){indicator.css({'opacity':1}).promise().then(function(){setTimeout(function(){indicator.animate({'top':-(iHeight-defaults.element.spaceBetweenRows)},200);},delay);});});},hideIndicator:function(){var indicator=$('.x-indicator');indicator.css({'opacity':0}).promise().then(function(){indicator.css({'left':0,'top':0});});},lazyLoadAsset:function(index){if(APP.GO.container.hasClass('x-lazyload-asset')){var clonePlaceHolder=$('.x-is-expanded'),placeholderImageWrap=clonePlaceHolder.find('.x-asset'),lowResImage=placeholderImageWrap.find('img'),highResImage=lowResImage.attr('data-src'),activeImageWrap=$(defaults.element.selector+'[data-index="'+index+'"]').find('.x-asset');if(lowResImage.attr('data-src')&&lowResImage.attr('data-src')!==null){lowResImage.attr('src',highResImage);lowResImage.one('load',function(){if(!activeImageWrap.hasClass('x-was-loaded')){placeholderImageWrap.addClass('x-is-loaded');}
var activeImage=activeImageWrap.find('img'),newActiveString=activeImage.attr('data-src');activeImage.attr('src',newActiveString).promise().then(function(){activeImageWrap.addClass('x-was-loaded');});}).each(function(){if(this.complete){$(this).trigger('load');}});}}},lazyLoadThumb:function(){if(APP.GO.container.hasClass('x-lazyload-thumbnail')){var placeholder=$('.'+defaults.placeholder.class),winBottom=APP.GO.winTop+APP.GO.winHeight;placeholder.each(function(){var $self=$(this),placeholderTop=$self.offset().top,placeholderBottom=placeholderTop-100,dataUntil=$self.attr('data-until'),inViewThumb=$(defaults.element.selector+'[data-group="'+dataUntil+'"]');if(placeholderBottom<=winBottom&&placeholderTop>=APP.GO.winTop){inViewThumb.each(function(){var img=$(this).find('> a > img'),newSource=img.attr('data-src');img.attr('src',newSource).promise().then(function(){inViewThumb.addClass('x-is-loaded');});});}});}},createButtons:function(updated){APP.GO.container.each(function(){var thisInstance=$(this),leftButton=$('<div class="x-arrow x-prev"></div>'),rightButton=$('<div class="x-arrow x-next"></div>'),closeButton=$('<div class="x-close"></div>');if(defaults.placeholder.showControls===true){if(updated===true){thisInstance.find('.'+defaults.placeholder.class).find('.x-updated-content').append(leftButton).append(rightButton).append(closeButton);}else{thisInstance.find('.'+defaults.placeholder.class).find('.'+defaults.placeholder.class+'-inner').append(leftButton).append(rightButton).append(closeButton);}}});},updateButtons:function(index,until){APP.GO.container.each(function(){var thisInstance=$(this);if(defaults.placeholder.showControls===true){if(index==='1'){thisInstance.find('.'+defaults.placeholder.class+'[data-until="'+until+'"]').find('.x-arrow.x-prev').addClass('x-arrow-disabled');}else if(index===APP.GO.itemLength.toString()){thisInstance.find('.'+defaults.placeholder.class+'[data-until="'+until+'"]').find('.x-arrow.x-next').addClass('x-arrow-disabled');}else{thisInstance.find('.'+defaults.placeholder.class).find('.x-arrow.x-prev, .x-arrow.x-next').removeClass('x-arrow-disabled');}}});APP.GO.onArrowClick();APP.GO.onKeyboardClick();},styleActivePlaceholder:function(){APP.GO.container.each(function(){var thisInstance=$(this),activeItem=thisInstance.find('.x-is-expanded'),horiMargins=(APP.GO.fixWidth)?0:defaults.placeholder.marginSides;activeItem.css({'margin-right':horiMargins,'margin-left':horiMargins,'width':'100%'}).css('width','-='+(horiMargins*2));});},onArrowClick:function(){$('.x-arrow').off('click').one('click',function(){var $self=$(this),activeItemIndex=$self.closest('.xpanda').find('.x-is-active').attr('data-index'),destination;if($self.hasClass('x-prev')){destination=parseInt(activeItemIndex,10)-1;}else{destination=parseInt(activeItemIndex,10)+1;}
$self.closest('.xpanda').find(defaults.element.selector+'[data-index="'+destination+'"]').trigger('click');});},onCloseClick:function(){$('.x-close').on('click',function(){$(this).closest('.xpanda').find('.x-is-active').trigger('click');});},onKeyboardClick:function(){APP.GO.doc.off('keydown').one('keydown',function(e){e.stopImmediatePropagation();var keyboardActiveInstance=$('.x-keyboard-active'),currentActive=keyboardActiveInstance.find('.x-is-active'),index=currentActive.attr('data-index'),columnCount=parseInt(keyboardActiveInstance.find('.x-item-wrap').attr('data-columns'),10),destination;if(currentActive.length>0){if(e.which===27){currentActive.trigger('click');}
if(e.which===37){destination=parseInt(index,10)-1;keyboardActiveInstance.find(defaults.element.selector+'[data-index="'+destination+'"]').trigger('click');}
if(e.which===38){destination=parseInt(index,10)-columnCount;keyboardActiveInstance.find(defaults.element.selector+'[data-index="'+destination+'"]').trigger('click');}
if(e.which===39){destination=parseInt(index,10)+1;keyboardActiveInstance.find(defaults.element.selector+'[data-index="'+destination+'"]').trigger('click');}
if(e.which===40){destination=parseInt(index,10)+columnCount;keyboardActiveInstance.find(defaults.element.selector+'[data-index="'+destination+'"]').trigger('click');}}});},historyLoad:function(){if(defaults.history){var href=window.location.href,getHook=href.substring(href.lastIndexOf('xpnd='),href.length).slice(5),item=APP.GO.container.find(String(defaults.element.selector)+'[data-path="'+getHook+'"]');item.trigger('click');}}};APP.GO.globalVar();APP.GO.win.on('load',function(){APP.GO.initialLoad();APP.GO.getColumns();APP.GO.createPlaceholders();APP.GO.styleItems();APP.GO.historyLoad();});APP.GO.win.on('resize',function(){APP.GO.throttleResizeFunc();});APP.GO.win.on('orientationchange',function(){window.location.reload(false);});APP.GO.win.on('scroll',function(){APP.GO.throttleScrollFunc();});APP.GO.win.on('popstate',function(){APP.GO.historyLoad();});APP.GO.onItemClick();APP.GO.onKeyboardClick();});};}(jQuery));
